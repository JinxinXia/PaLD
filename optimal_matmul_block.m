    ...
    for j = i:b:n
        ...
        if i == j % handle diagonal case differently
            ...
            for k=1:n
                ...
                C(Ib,k) = C(Ib,k) + sum(double((D(Ib,k) < D(k,Jb)) & (D(Ib,k) < Dij)) ./ Uij,2);
                C(Ib,k) = C(Ib,k) + .5*sum(double((D(Ib,k) == D(k,Jb)) & (D(Ib,k) < Dij)) ./ Uij,2);            
            end
            ...
            for k=1:n
                ...
                C(Jb,k) = C(Jb,k) + sum(double((D(Ib,k) > D(k,Jb)) & (D(k,Jb) < Dij)) ./ Uij)';
                C(Jb,k) = C(Jb,k) + .5*sum(double((D(Ib,k) == D(k,Jb)) & (D(k,Jb) < Dij)) ./ Uij)';
            end
        end
